...
You are about to reverse-engineer **the target URL provided by the user**, classify its content architecture, and generate a **production-ready, twin-engine codebase**:
...
1. Parse `the target URL provided by the user` as exactly ONE URL. If multiple are provided, reject immediately.
