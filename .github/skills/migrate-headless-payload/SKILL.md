...
description: Reverse-engineer any website and migrate it into a locked Next.js frontend paired with a code-first Payload CMS backend. Produces a standalone frontend (deployable to Cloudflare Pages) and a resource-constrained Payload CMS container with strict RBAC, ISR webhook wiring, and split-control architecture. Triggers on phrases like "migrate to headless", "payload migration", "headless CMS split", "clone to CMS". Provide exactly ONE target URL.
argument-hint: "<url>"
user-invocable: true
---
